"""
AdaptiveTrendV2 backtest wrapper — prefix-buffered runner.

WHY A NEW RUNNER (vs editing _adaptrend_run.py):
v1's runner loads only [start, end].  AdaptiveTrendV2 needs `fit_window_months`
of prior history INSIDE `self.data` for the first monthly re-opt to fire.
Without that prefix, every month of OOS would land in the "warmup" branch
(fit slice too short), v2 would never re-opt, and v2 ≡ v1.

This runner:
  1. Loads 15m bars from (start - fit_window_months) through end.
  2. Runs the Backtest on the FULL prefixed slice.
  3. Discards trades whose EntryTime < start when computing the published
     return / funding numbers (so the OOS comparison is over [start, end]
     just like v1).
  4. Reports v2's per-month (L, theta) choices for diagnostics.

Funding model is identical to v1: post-process trades via funding_cost_for_trades.

Used by tools/adaptrend_v2_sanity.py.  Not wired to live bot.
"""
...
def run(
    ...
    # CRITICAL: pass trade_start_ns so the strategy suppresses entries during
    # the prefix.  Without this, v2 would carry a December position into the
    # OOS window and contaminate the comparison with v1 (which starts flat).
    run_kwargs = dict(config)
    run_kwargs.setdefault("trade_start_ns", oos_start_ts.value)
    print(
        f"[adaptrend_v2] {start}..{end} prefix={prefix_months}mo "
        f"bars={len(data_15m_full)} cash={cash} cfg={run_kwargs}",
        file=sys.stderr,
    )
    ...
    # CRITICAL: filter trades whose EntryTime < oos_start_ts.  The prefix
    # SHOULD not generate trades (warmup branch in strategy.init_refit) but
    # be defensive in case any leak through (e.g. first re-opt fires before
    # full prefix is consumed).
    ...
    # Per-trade PnL sum gives the in-window return (gross of funding).
    ...
    # Equity-curve max DD over [oos_start, end] approximated from cumulative PnL.
    # (Not equal to backtesting.py's intrabar tracking; sufficient for cross-strategy comparison.)
    max_dd_pct = 0.0
    if trades_oos is not None and len(trades_oos) > 0 and "PnL" in trades_oos.columns:
    ...
    # Monthly choices log (from strategy instance).
    ...
    if save_trades is not None and trades_oos is not None and len(trades_oos) > 0:
        ...
        print(f"[adaptrend_v2] saved {len(out)} trades to {save_trades}", file=sys.stderr)
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(description="AdaptiveTrendV2 backtest with funding cost (prefix-buffered).")
    ...
    p.add_argument("--show-choices", action="store_true")
    ...
    result = run(
        ...
        label=args.label,
        return_monthly_choices=args.show_choices,
    )
