"""
AdaptiveTrendV2 + regime_gate_adx — improvement experiment runner.

Runs BOTH AdaptiveTrendV2 (base) AND AdaptiveTrendV2_regime_gate_adx
...
Architecture: clone of tools/_adaptrend_v2_run.py, parameterised on
the strategy class so we can run base and improvement back-to-back
under the IDENTICAL prefix-buffered harness (apples-to-apples).

Saves:
  reports/adaptrend_v2_imp_regime_gate_adx.json   (full result)
  reports/_adaptrend_v2_base11_trades.csv         (base trades, for diagnostics)
  reports/_adaptrend_v2_imp_adx_trades.csv        (improvement trades, for PSR)
...
from strategy.signals_adaptive_trend_v2_regime_gate_adx import (  # noqa: E402
    AdaptiveTrendV2_regime_gate_adx,
)
...
def main() -> None:
    base_csv = ROOT / "reports" / "_adaptrend_v2_base11_trades.csv"
    imp_csv = ROOT / "reports" / "_adaptrend_v2_imp_adx_trades.csv"
    ...
    print("=== AdaptiveTrendV2 base vs +regime_gate_adx — 11 OOS @ $1M ===", flush=True)
    print(
    ...
    for start, end, label in OOS_WINDOWS:
        ...
        i = run_one(
            AdaptiveTrendV2_regime_gate_adx, start, end, label, imp_csv
        )
        base_rows.append(b)
        imp_rows.append(i)
        print(
    ...
    out = {
        "improvement_id": "regime_gate_adx",
        "feature": "Entries gated on H6 ADX(14) > 25 (trend-only regime).",
        "windows": OOS_WINDOWS,
        "cash": _CASH,
        "base_rows": base_rows,
    ...
    out_path = ROOT / "reports" / "adaptrend_v2_imp_regime_gate_adx.json"
