"""AdaptiveTrendV1 + time_stop_24h ablation runner (post-fractional-refactor).

Compares AdaptiveTrendV1 (base, alpha=2.0) vs AdaptiveTrendV1_time_stop_24h
(same alpha, plus 24-hour hard time stop = 96 fifteen-minute bars after
entry) on the 5 OOS windows at $1M cash with PRICE_SCALE=0.001 fractional
sizing.

Promotion bar (from handoff):
  - Compounded equity must improve by >= +5pp over base (i.e. >= +50.52%).
  - PSR must NOT decrease vs base (base PSR = 0.905).

Writes:
  - reports/postfrac_adaptrend_v1_time_stop.json
  - reports/_postfrac_adaptrend_v1_time_stop_24h_<window>.csv per OOS window
  - reports/_postfrac_adaptrend_v1_time_stop_24h_baseRECHECK_<window>.csv per OOS window
  - reports/_postfrac_adaptrend_v1_time_stop_24h_aggregated.csv
  - reports/_postfrac_adaptrend_v1_time_stop_24h_baseRECHECK_aggregated.csv

PRICE_SCALE pattern mirrors tools/_postfrac_adaptrend_v1_adx.py — apples
to apples. NO funding net applied (matches the base's gross-of-funding
basis), so the delta vs base is purely the time-stop effect.

CAUTION: this is NOT the same lever as `tools/_adaptrend_v2_imp_time_stop_24h.py`
which set `max_hold_h6_bars=96` on V2 (96 H6 BARS = 24 DAYS). This runner
tests a real 24-HOUR cap on V1.
"""
...
from strategy.signals_adaptive_trend_v1_time_stop_24h import (  # noqa: E402
    AdaptiveTrendV1_time_stop_24h,
)
...
# header). The time stop is the ONLY variable changing here.
BASE_CONFIG = {"alpha": 2.0}
TEST_CONFIG = {"alpha": 2.0, "time_stop_24h": True, "time_stop_bars": 96}
...
def run_arm(
    ...
    print(f"[time_stop_ablation] running arm={arm_label} ({len(WINDOWS_5)} windows) ...",
          file=sys.stderr)
...
def verdict(base: dict, test: dict) -> dict:
    ...
    test_psr = test["canonical"]["psr_walkforward"]["psr_vs_hurdle"]
    base_psr_stitched = base["psr"]["psr_vs_hurdle"]
    test_psr_stitched = test["psr"]["psr_vs_hurdle"]
    base_sharpe = base["canonical"]["psr_walkforward"].get("point_sharpe")
    test_sharpe = test["canonical"]["psr_walkforward"].get("point_sharpe")
    ...
    return {
        ...
        "test_psr_stitched_legacy": test_psr_stitched,
        "base_point_sharpe":      base_sharpe,
        "test_point_sharpe":      test_sharpe,
        "base_trades":            base_trades,
        "test_trades":            test_trades,
        "delta_trades":           test_trades - base_trades,
...
def main() -> int:
    ...
    res_base = run_arm(
        ...
        AdaptiveTrendV1,
        BASE_CONFIG,
        csv_prefix="_postfrac_adaptrend_v1_time_stop_24h_baseRECHECK",
    )
    res_test = run_arm(
        "time_stop_24h",
        AdaptiveTrendV1_time_stop_24h,
        TEST_CONFIG,
        csv_prefix="_postfrac_adaptrend_v1_time_stop_24h",
    )

    v = verdict(res_base, res_test)

    result = {
        "experiment":     "adaptrend_v1_time_stop_24h",
        "base_strategy":  "strategy.signals_adaptive_trend:AdaptiveTrendV1",
        "test_strategy":  "strategy.signals_adaptive_trend_v1_time_stop_24h:AdaptiveTrendV1_time_stop_24h",
        "cash":           CASH,
        "commission":     COMMISSION,
        "margin":         MARGIN,
        ...
        "lever_definition": {
            ...
            "time_stop_hours":    24,
            "note": (
                "NOT the same lever as tools/_adaptrend_v2_imp_time_stop_24h.py, "
                "which used max_hold_h6_bars=96 = 24 DAYS. This runner caps holds "
                "at 96 fifteen-minute bars = 24 HOURS."
            ),
        },
        "elapsed_sec":    round(time.time() - t0, 2),
    }

    out_path = ROOT / "reports" / "postfrac_adaptrend_v1_time_stop.json"
    out_path.write_text(json.dumps(result, indent=2, default=str))
    print(
        f"[time_stop_ablation] verdict={v['decision']}  "
        ...
        f"trades {v['base_trades']} -> {v['test_trades']}",
        file=sys.stderr,
    )
    print(f"[time_stop_ablation] wrote {out_path}  ({time.time()-t0:.1f}s)",
          file=sys.stderr)
    return 0
