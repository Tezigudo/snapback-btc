"""
AdaptiveTrendV2 + session_volume_filter — improvement experiment runner.

Runs BOTH AdaptiveTrendV2 (base) AND AdaptiveTrendV2_session_volume_filter
...
Same harness as tools/_adaptrend_v2_imp_regime_gate_adx_run.py — only the
treatment strategy class differs.  Apples-to-apples comparison vs the
v2 base.

Saves:
  reports/adaptrend_v2_imp_session_volume_filter.json  (full result)
  reports/_adaptrend_v2_base11_svf_trades.csv          (base trades, scratch)
  reports/_adaptrend_v2_imp_svf_trades.csv             (improvement trades)
...
from strategy.signals_adaptive_trend_v2_session_volume_filter import (  # noqa: E402
    AdaptiveTrendV2_session_volume_filter,
)
...
def main() -> None:
    base_csv = ROOT / "reports" / "_adaptrend_v2_base11_svf_trades.csv"
    imp_csv = ROOT / "reports" / "_adaptrend_v2_imp_svf_trades.csv"
    ...
    print("=== AdaptiveTrendV2 base vs +session_volume_filter — 11 OOS @ $1M ===", flush=True)
    print(
    ...
    for start, end, label in OOS_WINDOWS:
        ...
        i = run_one(
            AdaptiveTrendV2_session_volume_filter, start, end, label, imp_csv
        )
        base_rows.append(b)
        imp_rows.append(i)
        print(
    ...
    out = {
        "improvement_id": "session_volume_filter",
        "feature": "Entries restricted to H6 closes inside [12:00, 22:00) UTC (London+NY overlap); of {0,6,12,18}, only {12,18}.",
        "windows": OOS_WINDOWS,
        "cash": _CASH,
        "base_rows": base_rows,
    ...
    out_path = ROOT / "reports" / "adaptrend_v2_imp_session_volume_filter.json"
