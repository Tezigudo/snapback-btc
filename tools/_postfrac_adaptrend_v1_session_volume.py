"""AdaptiveTrendV1 + session_volume_filter ablation runner (post-fractional).

Compares AdaptiveTrendV1 (base, alpha=2.0) vs AdaptiveTrendV1_session_volume
(same alpha, plus hour-of-day bucket volume gate at 1.2x of 60-day trailing
same-bucket mean) on the 5 OOS windows at $1M cash with PRICE_SCALE=0.001
fractional sizing.

Promotion bar (from handoff):
  - Compounded equity must improve by >= +5pp over base (>= +50.52%).
  - PSR must NOT decrease vs base (0.905).

Writes:
  - reports/postfrac_adaptrend_v1_session_volume.json
  - reports/_postfrac_adaptrend_v1_session_volume_filter_<window>.csv per OOS window
  - reports/_postfrac_adaptrend_v1_session_volume_filter_baseRECHECK_<window>.csv per window
  - reports/_postfrac_adaptrend_v1_session_volume_filter_aggregated.csv

PRICE_SCALE pattern mirrors tools/_postfrac_adaptrend_v1_adx.py — apples to
apples. NO funding net applied. The base arm here is a sanity-check that
must reproduce the +45.52% / PSR 0.905 baseline so the delta vs base
isolates the volume gate effect.

Volume column is preserved in the slice loader (Backtesting.py exposes any
extra columns as self.data.<Cap>; we capitalise to "Volume" so the strategy
can read it).
"""
...
from strategy.signals_adaptive_trend_v1_session_volume import (  # noqa: E402
...
# Hold alpha=2.0 to match postfrac base run.
...
def _load_slice_scaled(start: str, end: str) -> pd.DataFrame | None:
    ...
    # Volume left unscaled — it's a regime indicator, not a price.
    return sl
...
def run_arm(
    ...
    print(
        f"[session_volume_ablation] running arm={arm_label} ({len(WINDOWS_5)} windows) ...",
        file=sys.stderr,
    )
    ...
    if csv_prefix:
        ...
        print(
            f"  [{arm_label}] aggregated CSV -> {agg_csv.name}",
            file=sys.stderr,
        )
...
def main() -> int:
    ...
    res_base = run_arm(
        ...
        AdaptiveTrendV1,
        BASE_CONFIG,
        csv_prefix="_postfrac_adaptrend_v1_session_volume_filter_baseRECHECK",
    )
    res_test = run_arm(
        "session_volume",
        AdaptiveTrendV1_session_volume,
        TEST_CONFIG,
        csv_prefix="_postfrac_adaptrend_v1_session_volume_filter",
    )
    ...
    result = {
        ...
        "test_strategy":  "strategy.signals_adaptive_trend_v1_session_volume:AdaptiveTrendV1_session_volume",
        "cash":           CASH,
        "commission":     COMMISSION,
        "margin":         MARGIN,
    ...
    out_path = ROOT / "reports" / "postfrac_adaptrend_v1_session_volume.json"
    out_path.write_text(json.dumps(result, indent=2, default=str))
    print(
        f"[session_volume_ablation] verdict={v['decision']}  "
    ...
    print(
        f"[session_volume_ablation] wrote {out_path}  ({time.time()-t0:.1f}s)",
        file=sys.stderr,
    )
    return 0
